...
## 11.12.0
### Added
- Nullable property types in the PHP generators. A property declared `type: X | nil`, `type: X | null` or `type: X?` now generates an accessor whose type admits `null`, independently of `required` — so a property that is always present but may hold no value can finally be expressed. Supported for scalars, the `datetime` family, `object`, `file` and references to named types in both PHP generators, with the caveat that a Symfony-bundle `file` property maps to a `file` column type Doctrine does not provide, so the host application must register one; an `array` is nullable only for `integer`, `string` or `boolean` `items` and only in the REST client, the Symfony bundle keeping it a collection initialised to `[]`, every other `items` type being rejected. Setters are unchanged and remain inconsistent about `null`: those carrying a type declaration (`array`, `\DateTimeInterface`, a named class) reject it, while scalar, `object` and `file` setters accept and forward it. Unions of any other shape remain unsupported and are still rejected with `Did not found defined type`. The JavaScript generator cannot render nullable types and rejects such a contract naming the property and the constraint.
- `PropertyDefinition::allowsNullValue()`, which is true when the value may be null or the key may be absent. `isRequired()` continues to report the RAML `required` facet on its own.
### Fixed
- Getters for `object` properties in generated REST clients are no longer emitted at column 0. The `{% else -%}` branch of the default getter template trimmed the whitespace its sibling branches keep, so the one branch that renders `getByReference()` lost its body indent.
