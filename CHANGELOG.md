...
## 11.12.0
### Added
- Nullable property types in the PHP generators. A property declared `type: X | nil`, `type: X | null` or `type: X?` now generates an accessor whose type admits `null`, independently of `required` — so a property that is always present but may hold no value can finally be expressed. Supported for scalars, the `datetime` family, `object`, `file` and references to named types in both PHP generators; an `array` of a scalar `items` type is nullable in the REST client only, the Symfony bundle keeping it a collection initialised to `[]`, and an `array` of a named type is rejected because the getter cannot distinguish null from empty. Nullability applies mainly to the read side: generated setters still require a non-null argument for every type except `file`. Unions of any other shape remain unsupported and are still rejected with `Did not found defined type`. The JavaScript generator cannot render nullable types and rejects such a contract naming the property and the constraint.
