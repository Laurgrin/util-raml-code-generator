...
## 11.12.0
### Added
- Nullable property types in the PHP generators. A property declared `type: X | nil`, `type: X | null` or `type: X?` now generates an accessor whose type admits `null`, independently of `required` — so a property that is always present but may hold no value can finally be expressed. Supported for scalars, the `datetime` family and references to named types in both PHP generators; an `array` property with an `items` node is nullable in the REST client only, the Symfony bundle keeping it a collection initialised to `[]`. Nullability applies to the read side: generated setters still require a non-null argument. Unions of any other shape remain unsupported and are still rejected with `Did not found defined type`. The JavaScript generator cannot render nullable types and rejects such a contract naming the property and the constraint.
