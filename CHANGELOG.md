...
## 11.12.0
### Added
- Nullable property types in the PHP generators. A property declared `type: X | nil`, `type: X | null` or `type: X?` now generates an accessor whose type admits `null`, independently of `required` — so a property that is always present but may hold no value can finally be expressed. Supported for scalars, the `datetime` family, arrays declared with an `items` node, and references to named types. Nullability applies to the read side: generated setters still require a non-null argument. Unions of any other shape remain unsupported and are still rejected with `Did not found defined type`. The JavaScript generator cannot render nullable types and rejects them with `Did not found defined type`, quoting the declaration as written.
